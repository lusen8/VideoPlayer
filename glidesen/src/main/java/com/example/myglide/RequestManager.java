...
import static com.example.myglide.request.RequestOptions.decodeTypeOf;
import static com.example.myglide.request.RequestOptions.diskCacheStrategyOf;
import static com.example.myglide.request.RequestOptions.skipMemoryCacheOf;
...
import com.example.myglide.load.resource.drawable.DrawableTransitionOptions;
import com.example.myglide.load.resource.gif.GifDrawable;
import com.example.myglide.manager.ConnectivityMonitor;
import com.example.myglide.manager.ConnectivityMonitorFactory;
import com.example.myglide.manager.Lifecycle;
import com.example.myglide.manager.LifecycleListener;
import com.example.myglide.manager.RequestManagerTreeNode;
import com.example.myglide.manager.RequestTracker;
import com.example.myglide.manager.TargetTracker;
import com.example.myglide.request.BaseRequestOptions;
...
import com.example.myglide.request.target.ViewTarget;
...
/**
 * A class for managing and starting requests for MyGlide. Can use activity, fragment and connectivity
 ...
 * handling, use the static MyGlide.load methods with your Fragment or Activity.
 *
 * @see MyGlide#with(android.app.Activity)
 * @see MyGlide#with(android.support.v4.app.FragmentActivity)
 * @see MyGlide#with(android.app.Fragment)
 * @see MyGlide#with(android.support.v4.app.Fragment)
 * @see MyGlide#with(Context)
 */
public class RequestManager implements LifecycleListener {
    ...
    protected final MyGlide myGlide;
    ...
    public RequestManager(MyGlide myGlide, Lifecycle lifecycle, RequestManagerTreeNode treeNode) {
        this(myGlide, lifecycle, treeNode, new RequestTracker(), myGlide.getConnectivityMonitorFactory());
    }
    ...
    RequestManager(
        ...
        this.myGlide = myGlide;
        this.lifecycle = lifecycle;
        this.treeNode = treeNode;
        this.requestTracker = requestTracker;

        final Context context = myGlide.getGlideContext().getBaseContext();
        ...
        setRequestOptions(myGlide.getGlideContext().getDefaultRequestOptions());

        myGlide.registerRequestManager(this);
    }
    ...
    public void onTrimMemory(int level) {
        myGlide.getGlideContext().onTrimMemory(level);
    }
    ...
    public void onLowMemory() {
        myGlide.getGlideContext().onLowMemory();
    }
    ...
    public void onDestroy() {
        ...
        myGlide.unregisterRequestManager(this);
    }
     ...
     * com.example.myglide.load.ResourceDecoder}s that can decode any subclass of {@link Drawable}.
     *
     ...
     * com.example.myglide.load.engine.DiskCacheStrategy#DATA}. As a result, specifying a
     * {@link com.example.myglide.load.engine.DiskCacheStrategy} on this request is generally not
     * recommended.
     *
     ...
     * obtained from the media store (for local images/videos), or using MyGlide's disk cache
     ...
     * {@link com.example.myglide.load.ResourceDecoder}s
    ...
    public <ResourceType> RequestBuilder<ResourceType> as(Class<ResourceType> resourceClass) {
        return new RequestBuilder<>(myGlide, this, resourceClass);
    }

    /**
     * Cancel any pending loads MyGlide may have for the view and free any resources that may have been
     * loaded for the view.
     *
     * <p> Note that this will only work if {@link View#setTag(Object)} is not called on this view
     * outside of MyGlide. </p>
     *
     * @param view The view to cancel loads and free resources for.
     * @throws IllegalArgumentException if an object other than MyGlide's metadata is put as the view's
     *                                  tag.
     * @see #clear(Target)
     */
    ...
    /**
     * Cancel any pending loads MyGlide may have for the target and free any resources (such as
    ...
    private void untrackOrDelegate(Target<?> target) {
        ...
        if (!isOwnedByUs) {
            myGlide.removeFromManagers(target);
        }
    }
