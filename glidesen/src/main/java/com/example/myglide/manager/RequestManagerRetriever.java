package com.example.myglide.manager;
...
import com.example.myglide.RequestManager;
...
/**
 * A collection of static methods for creating new {@link com.example.myglide.RequestManager}s or
 * retrieving existing ones from activities and fragment.
 */
public class RequestManagerRetriever implements Handler.Callback {
  // Visible for testing.
  static final String FRAGMENT_TAG = "com.bumptech.myGlide.manager";
    ...
    if (applicationManager == null) {
      synchronized (this) {
        if (applicationManager == null) {
          ...
          // TODO(b/27524013): Factor out this MyGlide.get() call.
          MyGlide myGlide = MyGlide.get(context);
          applicationManager =
              factory.build(myGlide, new ApplicationLifecycle(), new EmptyRequestManagerTreeNode());
        }
      }
    }
  ...
  private RequestManager fragmentGet(Context context, android.app.FragmentManager fm,
    ...
    if (requestManager == null) {
      ...
      MyGlide myGlide = MyGlide.get(context);
      requestManager =
          factory.build(myGlide, current.getLifecycle(), current.getRequestManagerTreeNode());
  ...
  public interface RequestManagerFactory {
    RequestManager build(
            MyGlide myGlide, Lifecycle lifecycle, RequestManagerTreeNode requestManagerTreeNode);
  }

  private static final RequestManagerFactory DEFAULT_FACTORY = new RequestManagerFactory() {
    @Override
    public RequestManager build(MyGlide myGlide, Lifecycle lifecycle,
                                RequestManagerTreeNode requestManagerTreeNode) {
      return new RequestManager(myGlide, lifecycle, requestManagerTreeNode);
    }
  };
}
