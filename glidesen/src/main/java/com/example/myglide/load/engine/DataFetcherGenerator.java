...
/**
 * Generates a series of {@link com.example.myglide.load.data.DataFetcher DataFetchers} using
 * registered {@link com.example.myglide.load.model.ModelLoader ModelLoaders} and a model.
 */
interface DataFetcherGenerator {
  /**
   * Called when the generator has finished loading data from a
   * {@link com.example.myglide.load.data.DataFetcher}.
   */
  interface FetcherReadyCallback {

    /**
     * Requests that we call startNext() again on a MyGlide owned thread.
     */
    void reschedule();
